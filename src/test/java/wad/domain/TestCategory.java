...
//        fileObject.setContentLength(file.getSize());
    ...
    @Test
    public void testAddNewsItemToCategory() {
        Category category = categoryRepository.findByName("Luonto");

        category.addNewsItem(newsItem);
        assertTrue(category.getNews().contains(newsItem));
    }
    ...
    public void testCannotAddNewsItemTwice() {
        ...
        category.addNewsItem(newsItem);
        category.addNewsItem(newsItem);
        assertEquals(1, category.getNews().size());
    }
}
